...
def process_eroi_date(row_df, eroi_date, mz_min, mz_max, retention_start, retention_end):
    """
    根据m/z范围和保留时间范围筛选数据，并保存为Excel文件。

    参数:
        row_df (pd.DataFrame): 处理后的数据表格
        eroi_date (str): 输出Excel文件路径
    ...
    selected_df.to_excel(eroi_date, index=True)  # 保存为Excel文件
